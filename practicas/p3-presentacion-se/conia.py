...
# 1. BASE DE CONOCIMIENTO (las "reglas")
...
# Esta función se vale de las reglas, solo sabe cómo aplicar el conocimiento.
...
def motor_de_inferencia(hechos_usuario, reglas):
    ...
    for regla in reglas:
        ...
        for condicion, valor in condiciones.items():
            # Si un hecho del usuario no coincide con la condición de la regla, no aplica
            if condicion not in hechos_usuario or hechos_usuario[condicion] != valor:
                cumple = False
                break
        if cumple:
            # Si todas las condiciones coincidieron, hemos encontrado una recomendación
...
# Verifica
# Si cambiamos algún valor en los hechos, este modificará la lógica de la recomendación.
