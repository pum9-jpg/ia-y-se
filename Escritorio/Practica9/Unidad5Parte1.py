...
class AgenteAspiradora:
    """
    Un agente reactivo simple que toma decisiones basadas únicamente
    en la percepción actual.
    """
    def actuar(self, percepcion):
        ubicacion, estado = percepcion

        # Regla simple: SI la ubicación actual está SUCIA, ENTONCES ASPIRAR.
        if estado == 'Sucia':
            return 'aspirar'
        
        # Regla: SI está LIMPIA, ENTONCES MOVERSE.
        elif ubicacion == 'A':
            return 'ir_a_B'
        elif ubicacion == 'B':
            return 'ir_a_A'
        
        return 'no_hacer_nada' # Fallback
